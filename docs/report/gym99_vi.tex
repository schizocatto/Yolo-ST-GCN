...
Gym99 bao phủ bốn dụng cụ thể dục: Vault (VT, nhãn $[0,5]$, 6 lớp),
Floor Exercise (FX, nhãn $[6,40]$, 35 lớp), Balance Beam (BB, nhãn
$[41,73]$, 33 lớp), và Uneven Bars (UB, nhãn $[74,98]$, 25 lớp).
Các thực nghiệm trong bài báo này tập trung vào tập con \textbf{FX}.
...
mức độ mất cân bằng lớp thấp hơn đáng kể: tỉ lệ max/min là
$8\times$ (thay vì $26\times$) và Gini = 0,306 (thay vì 0,438), với
effective number of classes đạt 30,1/35. Hình~\ref{fig:fx_class_dist}
minh họa phân phối lớp và đường cong Lorenz của tập FX.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\columnwidth]{../figures/FX_train_class_distribution.png}
  \caption{Phân phối lớp tập con FX: (trái) số mẫu mỗi lớp sắp xếp
  giảm dần (train xanh / val cam); (giữa) histogram số mẫu/lớp; (phải)
  đường Lorenz -- Gini = 0,306.}
  \label{fig:fx_class_dist}
\end{figure}

\textbf{Phân tích không gian và chuyển động.}
Tọa độ keypoint gốc nằm trong không gian ảnh pixel ($1920 \times 1080$),
với tâm khung xương ở vị trí trung bình $(\bar{x} \approx 1088,\,
\bar{y} \approx 488)$~px. Hình~\ref{fig:fx_velocity} cho thấy các khớp
chi trên (cổ tay, khuỷu tay) và chi dưới (mắt cá chân, đầu gối) có
vận tốc trung bình cao hơn đáng kể so với các khớp trục thân (hông,
vai), phản ánh đặc trưng động học của bài thể dục tự do.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=\columnwidth]{../figures/FX_train_per_joint_velocity.png}
  \caption{Vận tốc tuyệt đối trung bình theo từng khớp COCO-18 trên tập
  huấn luyện FX. Đỏ = Q3 trở lên (hoạt động nhất); xanh dương = Q1 trở
  xuống (ít hoạt động nhất).}
  \label{fig:fx_velocity}
\end{figure}

\textbf{Khả năng phân tách lớp.}
Hình~\ref{fig:fx_pca} trình bày PCA hai chiều trên đặc trưng trung bình
thời gian ($36$ chiều) của 20 lớp phổ biến nhất. Sự chồng chéo lớp cao
trong không gian 2D xác nhận tính chất fine-grained của tập FX: các bài
tập tự do có cấu trúc khung xương rất tương đồng, sự khác biệt chủ yếu
nằm ở thứ tự và pha thời gian của động tác.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=0.9\columnwidth]{../figures/FX_train_pca.png}
  \caption{PCA hai chiều trên 20 lớp phổ biến nhất của tập FX (train).
  Sự chồng chéo cao giữa các lớp xác nhận tính chất fine-grained của
  bài toán phân loại.}
  \label{fig:fx_pca}
\end{figure}
...
Luồng joint nhận tensor $\mathbf{X}^{(j)} \in \mathbb{R}^{N \times 2 \times 48 \times 18}$
là tọa độ $(x,y)$ của các khớp. Luồng bone nhận tensor
$\mathbf{X}^{(b)} \in \mathbb{R}^{N \times 2 \times 48 \times 18}$ là
